...
export default function App() {
  ...
  const NAV = {
    admin: [
      ["dashboard", "Panel", "▦"], ["clientes", "Clientes", "◍"], ["prospectos", "Prospectos", "◎"],
      ["rutas", "Plan Visitas", "⇄"], ["visitas", "Visitas", "✓"],
      ["viajes", "Viajes y fondos", "✈"], ["agenda", "Agenda", "▤"],
      ["precios", "Listas de Precios", "₵"], ["marketing", "Marketing", "◈"],
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className="content">
          {activeView === "dashboard" && <Dashboard db={db} go={setView} />}
          {activeView === "clientes" && <Clientes db={db} />}
          {activeView === "prospectos" && <Prospectos db={db} />}
          {activeView === "rutas" && <Rutas db={db} go={setView} />}
          {activeView === "visitas" && <Visitas db={db} />}
...
function Dashboard({ db, go }) {
  ...
  // Revisitas pendientes (cada 15 días)
  const hayVisitas = vs.length > 0;
  const revisitas = cl.map((c) => {
    const ult = vs.filter((v) => v.clienteId === c.id).map((v) => v.fecha).sort().pop();
    const dias = ult ? daysBetween(ult, today()) : 999;
    return { c, ult, dias };
  }).filter((r) => r.ult && r.dias >= 15).sort((a, b) => b.dias - a.dias);
  ...
  return (
    <div className="stack">
      ...
      <div className="quick-access">
        ...
        <button className="qa" onClick={() => go("prospectos")}><span className="qa-ico">◎</span>Prospectos</button>
        ...
        <button className="qa" onClick={() => go("marketing")}><span className="qa-ico">◈</span>Marketing</button>
        <button className="qa" onClick={() => go("reportes")}><span className="qa-ico">▮</span>Reportes</button>
      </div>

      <div className="card">
        <div className="card-head">
          <h3>Recordatorio de revisita <span className="muted">(cada 15 días)</span></h3>
...
function emptyCliente(me) {
  return {
    ...
    nombre: "", razonSocial: "", nombreFantasia: "", rut: "", estado: "Lead", categoria: "3", tipo: "Tienda de repuestos",
...
function Clientes({ db }) {
  ...
  return (
    <div className="stack">
      ...
      {editing && <ClienteForm cliente={editing} onSave={save} onClose={() => setEditing(null)} db={db} />}
...
function ClienteForm({ cliente, onSave, onClose, db }) {
  const { me, users } = db;
    ...
    <Modal wide title={c.nombre || "Nuevo cliente"} onClose={onClose}>
      ...
      {tab === "datos" && (
        <div className="grid2">
          ...
          <Field label="Categoría (límite de crédito)">
      ...
      <div className="modal-foot merge-foot">
        ...
        <div className="foot-right">
          ...
          <Btn kind="primary" onClick={() => onSave(c)} disabled={!c.nombre}>Guardar cliente</Btn>
        </div>
      </div>
...
/* ---------- Prospectos ---------- */
function Prospectos({ db }) {
  const { me, clientes, setClientes, users } = db;
  const [editing, setEditing] = useState(null);
  const [merging, setMerging] = useState(null);
  const [q, setQ] = useState("");
  const visibles = clientes.filter((c) => (me.role === "admin" || c.vendedorId === me.id) && c.estado === "Prospecto")
    .filter((c) => !q || (c.nombre + c.comuna + c.region).toLowerCase().includes(q.toLowerCase()));

  const save = (c) => { setClientes((prev) => prev.map((x) => x.id === c.id ? c : x)); setEditing(null); };
  const convertir = (c) => {
    if (!confirm("¿Convertir “" + c.nombre + "” en cliente? Pasará a la lista de Clientes.")) return;
    setClientes((prev) => prev.map((x) => x.id === c.id ? { ...x, estado: "Cliente" } : x));
  };
  ...
  return (
    <div className="stack">
      <div className="toolbar">
        <input className="search" placeholder="Buscar prospecto, comuna o región…" value={q} onChange={(e) => setQ(e.target.value)} />
        <div className="muted small">{visibles.length} prospecto(s)</div>
      </div>
      {visibles.length === 0
        ? <Empty icon="◎" title="Sin prospectos" sub="Aquí aparecen los negocios por captar que aún no son clientes." />
        : (
          <div className="cards-grid">
            {visibles.map((c) => (
              <div key={c.id} className="cli-card" onClick={() => setEditing(c)}>
                <div className="cli-top">
                  <div className="cli-name">{c.nombre}</div>
                  <Badge tone="amber">Prospecto</Badge>
                </div>
                <div className="cli-meta">{c.segmentos?.join(", ") || c.tipo}</div>
                <div className="cli-meta">{[c.comuna, c.region].filter(Boolean).join(", ") || "Sin zona"}</div>
                {c.telefono && <div className="cli-meta">☎ {c.telefono}</div>}
                {me.role === "admin" && c.vendedorId &&
                  <div className="cli-meta">Vendedor: {users.find((u) => u.id === c.vendedorId)?.nombre || "—"}</div>}
                <div className="cli-actions" onClick={(e) => e.stopPropagation()}>
                  {(addrOf(c)) && <>
                    <Btn small as="a" href={wazeUrl(addrOf(c), coordOf(c))} kind="waze">Waze</Btn>
                    <Btn small as="a" href={mapsDir(addrOf(c))} kind="maps">Maps</Btn>
                  </>}
                </div>
                <div className="cli-actions" onClick={(e) => e.stopPropagation()}>
                  {!c.vendedorId && me.role !== "admin" && <Btn small onClick={() => asignarme(c)}>Tomar</Btn>}
                  <Btn small onClick={() => setMerging(c)}>⇄ Fusionar</Btn>
                  <Btn small kind="primary" onClick={() => convertir(c)}>Convertir a cliente</Btn>
                </div>
              </div>
            ))}
          </div>
...
function Rutas({ db, go }) {
  ...
  return (
    <div className="stack">
      ...
      {ficha && <ClienteForm cliente={ficha} onSave={saveCli} onClose={() => setFicha(null)} db={db} />}
    </div>
  );
}
...
function Visitas({ db }) {
  ...
  return (
    <div className="stack">
      ...
      <div className="card">
        <div className="card-head"><h3>Visitas programadas</h3></div>
              ...
              {programadas.map((x) => {
                ...
                return (
                  <div key={x.id} className="row">
                    <div>
                      <div className="row-t">{nombre(x.clienteId)} <Badge tone="amber">Programada</Badge></div>
                      <div className="row-s">{x.fecha} · {c?.comuna || "—"}</div>
                    </div>
                    <div className="row-actions col">
                      ...
                      <Btn small kind="primary" onClick={() => marcarVisitada(x)}>Registrar Visita</Btn>
                    </div>
                  </div>
                );
              })}
...
function Styles() {
  return (
    ...
    :root{
      --asphalt:#16181C; --carbon:#1E2127; --panel:#23272E; --panel2:#2A2F37;
      --line:#343A44; --txt:#ECEEF1; --muted:#8C95A3; --amber:#F2A33C; --amber-d:#C9821F;
      --green:#46C28B; --red:#E2685B; --blue:#5BA9E8; --waze:#33CCFF; --maps:#34A853;
    ...
    *{box-sizing:border-box}
    ...
    /* SIDEBAR */
    .side{width:248px;flex-shrink:0;background:var(--carbon);border-right:1px solid var(--line);
    ...
    .nav-i{display:flex;align-items:center;gap:11px;width:100%;border:0;background:transparent;color:var(--muted);
    ...
    .nav-i.on{background:var(--panel2);color:var(--txt)}
    .nav-i.on .nav-ico{color:var(--amber)}
    ...
    .me-n{font-weight:600;font-size:13px}.me-r{font-size:11px;color:var(--muted)}
    .logout{width:100%;background:transparent;border:1px solid var(--line);color:var(--muted);
